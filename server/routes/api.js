...
const ObjectID = require('mongodb').ObjectID; // research!
mongoose.Promise = global.Promise;             // research!
const Schema = mongoose.Schema;									
const usersSchema = new Schema({userName: String, email: String, password: String, token: String});
const cartSchema = new Schema({token: String, cart: {type: Array, 'default': []}}); // research
...
const sendError = (err, res) => {
    response.status = 501;
    response.message = typeof err == 'object' ? err.message : err;  // Research 'Object'
    res.status(501).json(response);
};

//*********** Response handling **********
let response = { // Research let
    status: 200,
    data: [],
    count: 0,
    message: null
...
router.get('/getUserByEmail', (req, resp) => {
       ...
       connection((db) => {
        ...
        model.find(query)
            .then((user) => {
                ...
                resp.json(response); // Update the header in the json response
            })
            .catch((err) => {
                sendError(err, resp);
            });
 ...
 router.post('/validatePassword', (req, resp) => {
   ...
   connection((db) => {
    ...
    model.find(query)
        .then((user) => {
            if(user.length > 0) { // user[0] b/c given there is only one unique email registered
                decryptedPass = PasswordModule.Decrypto(PasswordModule.Decrypto(user[0].password));
                decryptedPass = decryptedPass.replace(/\s/g,''); // remove space
...
/************************* ends **********/
 ...
 router.put('/expandUserOrderHistory', (req, resp) => {
    connection((db) => {
     ...
     const query = {'token': token}, where = { $set: { 'cart':item }}; // research set
